...
- DataSource 适配接口、内存 Mock Adapter 和消息状态 controller
- 本地草稿、隐藏消息、偏好设置和布局存储
- demo 数据和轻量扩展注册表
...
模板提供 `createMemoryChatDataSource` 作为本地 Mock，`useChatDataController` 负责分页、乐观发送、失败重试、去重和服务端回执合并。
