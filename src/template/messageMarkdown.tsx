...
export function parseMarkdownBlocks(value: string): MarkdownBlock[] {
	...
	while (index < lines.length) {
		...
		while (
			index < lines.length &&
			lines[index].trim() &&
			!matchCodeFenceStart(lines[index]) &&
			!/^>\s?/.test(lines[index]) &&
...
function matchCodeFenceStart(line: string) {
...
function renderMarkdownText(value: string, keyPrefix: string) {
	...
	while (match) {
		...
		if (match[1] !== undefined && match[2]) {
			...
			match[5] !== undefined &&
			isSingleBacktickSpan(value, match.index, match.index + match[0].length)
		) {
			nodes.push(
				<code
			...
			nodes.push(
				<Fragment key={`${keyPrefix}-code-raw-${match.index}`}>
					{match[0]}
				</Fragment>,
			);
...
function isSingleBacktickSpan(value: string, start: number, end: number) {
