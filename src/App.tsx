...
export function TemplateDemoApp() {
	...
	const dataSource = useMemo(
		() =>
			createMemoryChatDataSource({
				...
				pageSize: 18,
				latencyMs: 180,
				shouldFailSend: (input) =>
					"body" in input && input.body.trim().toLowerCase() === "/fail",
			}),
		[initialData],
	);
	...
	const activeMessagePage = activeConversationId
		? (messagePages[activeConversationId] ?? null)
		: null;
	...
	async function sendMessage(body: string) {
		...
		await sendDataMessage(shell.activeConversationId, body);
	}
	...
	async function handleMessageAction(message: Message, action: MessageAction) {
		...
		updateConversations((current) =>
			updateConversationLastMessage(current, reply),
		);
	}
		...
		<ChatShell
			...
			mainContent={
				<ChatMainContent
					...
					loadingMessages={Boolean(activeMessagePage?.loading)}
					loadingMoreMessages={Boolean(activeMessagePage?.loadingMore)}
					hasMoreMessages={Boolean(activeMessagePage?.hasMore)}
					conversationPrefs={conversationPrefs}
					drafts={drafts}
					contacts={contacts}
					query={shell.query}
